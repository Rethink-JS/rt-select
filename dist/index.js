/*! @rethink-js/rt-select v1.0.0 | MIT */
(() => {
  // src/index.js
  (function() {
    var RT_NS = "rtSelect";
    if (window[RT_NS] && window[RT_NS].__initialized) return;
    var DEFAULT_CHOICES_SCRIPT = "https://cdn.jsdelivr.net/npm/choices.js@11.1.0/public/assets/scripts/choices.min.js";
    var DEFAULT_CHOICES_STYLE = "https://cdn.jsdelivr.net/npm/choices.js@11.1.0/public/assets/styles/choices.min.css";
    ...
    function assignUID(el, attr2) {
      if (!el.getAttribute(attr2)) el.setAttribute(attr2, uid());
    ...
    function isSelect(el) {
      return !!(el && el.nodeType === 1 && String(el.tagName || "").toLowerCase() === "select");
    }
    ...
    function loadChoicesScript(src) {
      ...
      choicesScriptPromise = new Promise(function(resolve, reject) {
        var existing = document.querySelector('script[data-rt-select-choices="true"]') || document.querySelector('script[src*="choices"]');
        if (existing) {
    ...
    function loadChoicesStyle(src, shouldLoad) {
      ...
      choicesStylePromise = new Promise(function(resolve) {
        ...
        var existing = document.querySelector('link[data-rt-select-choices-css="true"]') || Array.from(document.querySelectorAll('link[rel="stylesheet"]')).filter(
          function(link) {
    ...
    function getOptions(select) {
      ...
      var renderChoiceLimit = parseNum(attr(select, "render-choice-limit"), void 0);
      if (renderChoiceLimit !== void 0) options.renderChoiceLimit = renderChoiceLimit;
      var searchResultLimit = parseNum(attr(select, "search-result-limit"), void 0);
      if (searchResultLimit !== void 0) options.searchResultLimit = searchResultLimit;
      var searchFloor = parseNum(attr(select, "search-floor"), void 0);
      if (searchFloor !== void 0) options.searchFloor = searchFloor;
      var maxItemCount = parseNum(attr(select, "max-item-count"), void 0);
      if (maxItemCount !== void 0) options.maxItemCount = maxItemCount;
      ...
      if (singleModeForMultiSelect !== void 0)
    ...
    Select.prototype.shouldUpgrade = function() {
      ...
      if (this.conf.mode === "all" || this.conf.mode === "any" || this.conf.mode === "custom" || this.conf.mode === "choices" || this.conf.mode === "choice" || this.conf.mode === "enhance" || this.conf.mode === "enhanced" || this.conf.mode === "always" || this.conf.mode === "force" || this.conf.mode === "true" || this.conf.mode === "1") {
        return true;
      }
      if (this.conf.mode === "none" || this.conf.mode === "native" || this.conf.mode === "off" || this.conf.mode === "false" || this.conf.mode === "0") {
        return false;
      }
    ...
    Select.prototype.findInner = function() {
      if (this.choices && this.choices.containerInner && this.choices.containerInner.element) {
    ...
    Select.prototype.applyStyles = function() {
      ...
      var css = '[data-rt-select-choice-uid="' + outerUID + '"]{box-sizing:border-box;width:100%;max-width:100%;margin-bottom:var(--rt-select-margin-bottom,0px);font-size:var(--rt-select-font-size,inherit);color:var(--rt-select-text,#111)}[data-rt-select-choice-uid="' + outerUID + '"] *{box-sizing:border-box}[data-rt-select-choice-uid="' + outerUID + '"].is-open{z-index:var(--rt-select-z-index,999)}[data-rt-select-choice-uid="' + outerUID + '"] .choices__inner{width:100%;min-height:var(--rt-select-min-height,44px);background:var(--rt-select-bg,#fff);border:var(--rt-select-border-width,1px) solid var(--rt-select-border,#d0d5dd);border-radius:var(--rt-select-radius,8px);box-shadow:var(--rt-select-shadow,none);color:var(--rt-select-text,#111);font-size:var(--rt-select-font-size,inherit);line-height:1.4;padding:var(--rt-select-padding-y,10px) var(--rt-select-padding-x,12px);transition:border-color .2s ease,box-shadow .2s ease,background-color .2s ease}[data-rt-select-choice-uid="' + outerUID + '"].is-focused .choices__inner,[data-rt-select-choice-uid="' + outerUID + '"].is-open .choices__inner{border-color:var(--rt-select-focus,#111);box-shadow:var(--rt-select-focus-shadow,none)}[data-rt-select-choice-uid="' + outerUID + '"].is-disabled .choices__inner{background:var(--rt-select-bg-disabled,#f5f5f5);cursor:not-allowed;opacity:.75}[data-rt-select-choice-uid="' + outerUID + '"][data-rt-select-invalid="true"] .choices__inner{border-color:var(--rt-select-invalid,#d92d20)}[data-rt-select-choice-uid="' + outerUID + '"] .choices__list--single{padding:0 calc(var(--rt-select-arrow-right,14px) + var(--rt-select-arrow-size,5px) + 8px) 0 0}[data-rt-select-choice-uid="' + outerUID + '"] .choices__placeholder{opacity:1;color:var(--rt-select-placeholder,#777)}[data-rt-select-choice-uid="' + outerUID + '"] .choices__input{background:transparent;color:var(--rt-select-text,#111);font-size:var(--rt-select-font-size,inherit);line-height:1.35;margin:0;min-width:9ch}[data-rt-select-choice-uid="' + outerUID + '"] .choices__input::placeholder{color:var(--rt-select-placeholder,#777);opacity:1}[data-rt-select-choice-uid="' + outerUID + '"][data-rt-select-multiple="true"] .choices__inner{display:flex;align-items:center;min-height:var(--rt-select-min-height,44px);padding:var(--rt-select-multi-padding-y,7px) var(--rt-select-multi-padding-x,8px)}[data-rt-select-choice-uid="' + outerUID + '"][data-rt-select-multiple="true"] .choices__list--multiple{display:flex;align-items:center;align-content:center;gap:var(--rt-select-chip-gap,6px);flex-wrap:wrap;width:100%;padding:0;margin:0}[data-rt-select-choice-uid="' + outerUID + '"][data-rt-select-multiple="true"] .choices__input{display:inline-flex;align-items:center;flex:1 1 140px;width:auto!important;max-width:100%;min-height:28px;padding:0 4px;margin:0;border:0!important;box-shadow:none!important}[data-rt-select-choice-uid="' + outerUID + '"][data-rt-select-multiple="true"] .choices__list--multiple .choices__item{display:inline-flex;align-items:center;max-width:100%;min-height:28px;background:var(--rt-select-tag-bg,#111);border:1px solid var(--rt-select-tag-border,#111);border-radius:var(--rt-select-tag-radius,999px);color:var(--rt-select-tag-color,#fff);font-size:calc(var(--rt-select-font-size,14px) * .9);font-weight:500;line-height:1.15;margin:0;padding:var(--rt-select-chip-padding-y,5px) var(--rt-select-chip-padding-x,9px);vertical-align:middle;word-break:break-word}[data-rt-select-choice-uid="' + outerUID + '"][data-rt-select-multiple="true"] .choices__button{position:relative;width:16px;height:16px;min-width:16px;margin:0 0 0 7px;padding:0;border:0;border-left:0!important;background-size:8px;opacity:.85}[data-rt-select-choice-uid="' + outerUID + '"] .choices__list--dropdown,[data-rt-select-choice-uid="' + outerUID + '"] .choices__list[aria-expanded]{background:var(--rt-select-dropdown-bg,#fff);border:var(--rt-select-border-width,1px) solid var(--rt-select-border,#d0d5dd);border-radius:var(--rt-select-dropdown-radius,8px);box-shadow:var(--rt-select-dropdown-shadow,0 12px 28px rgba(16,24,40,.12));color:var(--rt-select-text,#111);margin-top:6px;overflow:hidden;z-index:var(--rt-select-z-index,999)}[data-rt-select-choice-uid="' + outerUID + '"] .choices__list--dropdown .choices__list,[data-rt-select-choice-uid="' + outerUID + '"] .choices__list[aria-expanded] .choices__list{max-height:var(--rt-select-dropdown-max-height,260px);overflow:auto;-webkit-overflow-scrolling:touch}[data-rt-select-choice-uid="' + outerUID + '"] .choices__list--dropdown .choices__item,[data-rt-select-choice-uid="' + outerUID + '"] .choices__list[aria-expanded] .choices__item{font-size:var(--rt-select-font-size,inherit);line-height:1.35;padding:10px 12px;color:var(--rt-select-text,#111)}[data-rt-select-choice-uid="' + outerUID + '"] .choices__list--dropdown .choices__item--selectable.is-highlighted,[data-rt-select-choice-uid="' + outerUID + '"] .choices__list[aria-expanded] .choices__item--selectable.is-highlighted{background:var(--rt-select-highlight-bg,#f2f4f7);color:var(--rt-select-highlight-color,#111)}[data-rt-select-choice-uid="' + outerUID + '"] .choices__list--dropdown .choices__item[aria-selected="true"],[data-rt-select-choice-uid="' + outerUID + '"] .choices__list[aria-expanded] .choices__item[aria-selected="true"]{background:var(--rt-select-selected-bg,#eef4ff);color:var(--rt-select-selected-color,#111)}[data-rt-select-choice-uid="' + outerUID + '"] .choices__heading{border-bottom-color:var(--rt-select-border,#d0d5dd);color:var(--rt-select-placeholder,#777);font-size:calc(var(--rt-select-font-size,14px) * .85);font-weight:600;padding:10px 12px}[data-rt-select-choice-uid="' + outerUID + '"] .choices__item--disabled{opacity:.5;cursor:not-allowed}[data-rt-select-choice-uid="' + outerUID + '"][data-type*="select-one"]::after{border-color:var(--rt-select-arrow,#111) transparent transparent transparent;border-width:var(--rt-select-arrow-size,5px);right:var(--rt-select-arrow-right,14px);margin-top:calc(var(--rt-select-arrow-size,5px) * -.35)}[data-rt-select-choice-uid="' + outerUID + '"][data-type*="select-one"].is-open::after{border-color:transparent transparent var(--rt-select-arrow,#111) transparent;margin-top:calc(var(--rt-select-arrow-size,5px) * -.85)}[data-rt-select-choice-uid="' + outerUID + '"][data-type*="select-multiple"]::after{display:none!important}';
    ...
    Select.prototype.bind = function(el, type, fn, opts) {
      ...
      this.eventBindings.push({ el, type, fn, opts });
    };
    ...
    Select.prototype.init = function() {
      ...
      loadChoicesAssets(this.conf.scriptSrc, this.conf.styleSrc, this.conf.loadStyle).then(function() {
